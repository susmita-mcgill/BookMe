...
async function openRestaurant(r) {
  Object.assign(state, { restaurant: r, cart: [], order: null, error: "", showWhy: new Set(), showCustomize: new Set(), checkedIn: false });
            ...
            <div class="card dish-card ${d.fit.level === "blocked" ? "is-blocked" : ""}">
              ...
              <div class="dish-body">
                <div class="dish-head">
                  ...
                  ${d.fit.level === "blocked" ? `<span class="pill bad">${esc(d.fit.reason)}</span>` : d.parts ? `<button class="pill accent num pill-btn" data-why="${d.item_id}" aria-label="See why this dish scored ${d.match}%">${d.match}% match</button>` : `<span class="pill accent num">${d.match}% match</span>`}
                </div>
...
function resetDiner() {
  Object.assign(state, { restaurant: null, tables: [], menu: null, cart: [], order: null, error: "", showWhy: new Set(), showCustomize: new Set(), checkedIn: false, party: 3, slot: "7:30", seatZone: null, tipPct: 18, split: "even", assign: {}, review: { stars: 0, text: "", skip: new Set() }, history: [], current: null, mode: "diner" });
